...
namespace EbuyNetwork.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ActionResult Index(int? category_id, string keyword, int? startPrice, int? endPrice,int? page)
        {
            ...
            qTotal.CommandText = "SELECT COUNT([Item].id) AS total FROM [Item] LEFT JOIN [Category] ON [Item].category_id=[Category].id WHERE ([Item].sold=0 OR [Item].sold=1)  " + filter + "";
            ...
            q.CommandType = CommandType.Text;
            q.CommandText = "SELECT [Category].name AS category_name, [Category].item_count AS category_item_count,[Item].* FROM [Item] LEFT JOIN [Category] ON [Item].category_id=[Category].id WHERE ([Item].sold=0 OR [Item].sold=1) " + filter + " ORDER BY [Item].id DESC OFFSET "+page*16+" ROWS FETCH NEXT 16 ROWS ONLY";
