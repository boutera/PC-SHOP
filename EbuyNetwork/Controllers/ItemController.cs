...
namespace EbuyNetwork.Controllers
{
    public class ItemController : Controller
    {
        ...
        public ActionResult Index(int item_id)
        {
            ...
            q.CommandText = "SELECT TOP 1 [User].id AS seller_id, [User].firstname,[User].lastname, [User].email,[User].address,[User].CIN,[Category].name AS category_name, [Category].item_count AS category_item_count,[Item].* FROM [Item] LEFT JOIN [Category] ON [Item].category_id=[Category].id LEFT JOIN [User] ON [Item].user_id=[User].id WHERE [Item].id='"+item_id+"'";
